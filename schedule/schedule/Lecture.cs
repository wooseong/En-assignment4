...
namespace schedule
{
    public class Lecture
    {
        ...
        public bool SearchLectureWith() // 강의 출력을 위한 세부사항 설정
        {
            ...
            if (mainMenuNumber.Equals('2')) AddLecture(2);
            else if (mainMenuNumber.Equals('5')) AddLecture(5);
        ...
        public List<string> AddLecture(int menuNumber) // menuNumber 2 : 수강신청, 5 : 관심과목 추가
        {
            do
            {
                Console.Clear();
                ...
                for (int i = 1; i <= rowCount; i++)
                {
                    ...
                    if ((excelRange.Cells[i, 3].Value2.ToString.Equal(subjectNumber)) && (excelRange.Cells[i, 4].Value2.ToString.Equal(subjectclass)))
                    {
                        ...
                        {
                            if (menuNumber == 2)
                                selectLecture.Add(excelRange.Cells[i, j].Value2.ToString());//수강신청
                            else if (menuNumber == 5)
                                selectIntrestLecture.Add(excelRange.Cells[i, j].Value2.ToString());//관심과목담기
                        }
